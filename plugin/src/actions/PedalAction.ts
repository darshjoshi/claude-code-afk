...
export function setPedalCore(c: PluginCore): void {
  core = c;
}

@action({ UUID: "com.claude.code-control.pedal" })
export class PedalAction extends SingletonAction {
  override async onWillAppear(ev: WillAppearEvent): Promise<void> {
    const actionId = (ev.payload.settings.actionId as string) || "pedalAbort";
